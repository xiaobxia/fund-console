...
const FixedInvestment = {
  ...
  formatData: function(list) {
    ...
    list.forEach((item, index) => {
      const value = Math.abs(numberUtil.countDifferenceRate(item.kline.close, item.kline.preClose))
      const value2 = Math.abs(numberUtil.countDifferenceRate(item.kline.high, item.kline.low))
    ...
    const a = (all2 / count2) / 2
    const c = all / count
    const threshold = numberUtil.keepTwoDecimals((a + c) / 2)
